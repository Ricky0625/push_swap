...
/*   Updated: 2022/08/26 12:12:28 by wricky-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * 3. Get the median of the sorted copy. This is to make sure that when
 ...
 *    median. If it's greater than median, RA.
 * 5. OPTIMIZATION. If the pushed item is less than the "median of median",
 ...
 * 7. Call itself again.
**/
void	push_chunk(t_stkgrp *stacks, int num_of_arg, int chunk)
{
	t_stack	*a;
	t_stack	*b;
	int		*copy;
	...
	while (b->btm + 1 < chunk)
	{
		if (peek(a) < copy[median])
		{
			p_instr(stacks, PB);
			if (peek(b) < copy[median / 2])
				r_instr(stacks, RB, 1);
		}
		else
			r_instr(stacks, RA, 1);
	}
	num_of_arg -= (num_of_arg / 2);
	chunk += (num_of_arg / 2);
